...
func _ready() -> void:
	...
	# ===== NEU: Positioniere Spieler auf Terrain! =====
	var world_gen = arena.get_node_or_null("Tiles/WorldGeneration")
	if world_gen:
		print("DEBUG: Positioning participants on procedural terrain...")
		TacticsLevelSetup.setup_participants_on_procedural_terrain(arena, participant, world_gen)
	else:
		print("DEBUG: WorldGeneration not found, using default positions")
