...
## Arena Type: "STATIC" (test_level) oder "PROCEDURAL" (procedural_level)
@export var arena_type: String = "STATIC"
...
func configure_tiles() -> void:
	# Nur für test_level mit statischen Tiles!
	var _tiles: Node3D = get_node_or_null("Tiles") as Node3D
	if not _tiles:
		print("DEBUG: No Tiles node found (Procedural Level?)")
		return
	
	# Prüfe ob WorldGeneration existiert
	var world_gen = _tiles.get_node_or_null("WorldGeneration")
	if world_gen:
		print("✓ Procedural Level detected!")
		return  # WorldGeneration hat collisions bereits!
	
	# Static Level handling - einfach nichts machen
	print("✓ Static Level - using existing tiles")
